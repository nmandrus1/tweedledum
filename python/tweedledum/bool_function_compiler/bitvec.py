...
class BitVec(object):
    ...
    def __init__(self, length, value=0):
        ...
        if isinstance(value, int):
            ...
            if len(value) > length:
                raise TypeError(
                    f"[BitVec] String requires a bit vector of "
    ...
    def __eq__(self, other):
        if isinstance(other, BitVec):
            return BitVec(
                1, self._length == other._length and self._value == other._value
            )
    ...
    def __ne__(self, other):
        if isinstance(other, BitVec):
            return BitVec(
                1, self._length != other._length or self._value != other._value
            )
    ...
    def __setitem__(self, index, value):
        if isinstance(index, slice):
            ...
            if (i - j) != value._length:
                raise ValueError(
                    "[BitVec] Assignment requires BitVec of with the same length\n"
                )
            limit = 1 << (i - j)
            mask = (limit - 1) << j
            self._value &= ~mask
            ...
            if isinstance(value, BitVec):
                ...
                raise ValueError(
                    "[BitVec] Single element assignment requires "
